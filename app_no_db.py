...
def init_db():
    ...
    if not c.fetchone():
        ...
        )  # 60s=1min, 900s=15min, 3600s=1hour
...
class DataAggregator:
    ...
    def add_data_point(
        self, temp1, temp2, pressure1, pressure2, power, flow_coefficient
    ):
    ...
    def get_aggregated_data(self, interval_name, interval_seconds):
        ...
        prev_interval_start = current_interval_start - timedelta(
        ...
        interval_points = [
            dp
            for dp in self.data_points[interval_name]
        ...
        if interval_points:
            avg_data = {
                "temp1": sum(dp["temp1"] for dp in interval_points)
                / len(interval_points),
                "temp2": sum(dp["temp2"] for dp in interval_points)
                / len(interval_points),
                "pressure1": sum(dp["pressure1"] for dp in interval_points)
                / len(interval_points),
                "pressure2": sum(dp["pressure2"] for dp in interval_points)
                / len(interval_points),
                "power": sum(dp["power"] for dp in interval_points)
                / len(interval_points),
                "kw_ton": sum(dp["kw_ton"] for dp in interval_points)
                / len(interval_points),
                "cooling_tons": sum(dp["cooling_tons"] for dp in interval_points)
                / len(interval_points),
                "flow_rate": sum(dp["flow_rate"] for dp in interval_points)
                / len(interval_points),
            ...
            print(
                f"Aggregating {interval_name}: {len(interval_points)} points over {interval_seconds} seconds"
            )
            return avg_data
...
def get_data(interval):
    try:
        ...
        c.execute(
            "SELECT interval1_seconds, interval2_seconds, interval3_seconds FROM config WHERE id = 1"
        )
        intervals = c.fetchone()

        c.execute(
            """SELECT timestamp, kw_ton, 
